...
pttui_buffer_is_need_sync_ne(PttUIState *expected_state, PttUIState *current_state, PttUIBufferInfo *buffer_info, FileInfo *file_info)
{
    ...
    if(n_buffer >= file_info->n_total_line) {
        return false;
    }

    if(n_buffer < N_EXTEND_PTTUI_BUFFER) {
        return true;
    }
...
_sync_pttui_buffer_info_get_buffer(PttUIState *state, PttUIBuffer *current_buffer, bool is_pre, PttUIBuffer **new_buffer, PttUIBufferInfo *buffer_info)
{
    ...
    fprintf(stderr, "pttui_buffer._sync_pttui_buffer_info_get_buffer: after-while\n");
...
_pttui_buffer_init_buffer_no_buf_from_file_info(PttUIState *state, FileInfo *file_info, PttUIBuffer **buffer)
{
    *buffer = malloc(sizeof(PttUIBuffer));
    ...
    if(!error_code2) {
        ...
        p_buffer->storage_type = p_comment_info->storage_type;
    }
...
_extend_pttui_buffer_extend_pre_buffer_no_buf_core(PttUIBuffer *current_buffer, FileInfo *file_info, PttUIBuffer **new_buffer)
{
    ...
    *new_buffer = malloc(sizeof(PttUIBuffer));
    ...
    error_code = file_info_get_pre_line(file_info, current_buffer->the_id, current_buffer->content_type, current_buffer->block_offset, current_buffer->line_offset, current_buffer->comment_offset, tmp->the_id, &tmp->content_type, &tmp->block_offset, &tmp->line_offset, &tmp->comment_offset, &tmp->storage_type);
...
_extend_pttui_buffer_extend_next_buffer_no_buf_core(PttUIBuffer *current_buffer, FileInfo *file_info, PttUIBuffer **new_buffer)
{
    ...
    *new_buffer = malloc(sizeof(PttUIBuffer));
    ...
    error_code = file_info_get_next_line(file_info, current_buffer->the_id, current_buffer->content_type, current_buffer->block_offset, current_buffer->line_offset, current_buffer->comment_offset, tmp->the_id, &tmp->content_type, &tmp->block_offset, &tmp->line_offset, &tmp->comment_offset, &tmp->storage_type);
...
_reset_pttui_buffer_info(PttUIBufferInfo *buffer_info, FileInfo *file_info)
{
    ...
    fprintf(stderr, "pttui_buffer._reset_pttui_buffer_info: file_info:\n");
    log_file_info(file_info, "pttui_buffer._reset_pttui_buffer_info");

    fprintf(stderr, "pttui_buffer._reset_pttui_buffer_info: buffer_info:\n");
    log_pttui_buffer_info(buffer_info, "pttui_buffer._reset_pttui_buffer_info");
    ...
    for(; p_buffer; p_buffer = p_buffer->next) {
        ...
        if(p_buffer->block_offset == 0 && p_buffer->line_offset == 0) {
            error_code2 = _pttui_buffer_set_load_line_from_content_block_info(p_buffer, p_content_block);
            if(!error_code && error_code2) error_code = error_code2;
            error_code2 = _pttui_buffer_set_file_offset_from_content_block_info(p_buffer, p_content_block);
            if(!error_code && error_code2) error_code = error_code2;
...
Err
log_pttui_buffer_info(PttUIBufferInfo *buffer_info, char *prompt)
{
    fprintf(stderr, "%s: n_to_delete: %d n_new: %d is_to_save: %d is_saved: %d n_buffer: %d n_to_delete_buffer: %d\n", prompt, buffer_info->n_to_delete, buffer_info->n_new, buffer_info->is_to_save, buffer_info->is_saved, buffer_info->n_buffer, buffer_info->n_to_delete_buffer);

    PttUIBuffer *p_buffer = buffer_info->head;
    int n_buffer = buffer_info->n_buffer;
    int i = 0;
    char *disp_uuid = NULL;
