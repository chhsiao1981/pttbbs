...
_pttui_resource_dict_get_content_block_from_file_core(PttUIBuffer *buffer, PttUIResourceDict *resource_dict)
{
    ...
    fprintf(stderr, "pttui_resource_dict._pttui_resource_dict_get_content_block_from_file_core: after file-get-data: e: %d\n", error_code);
    ...
    fprintf(stderr, "pttui_resource_dict._pttui_resource_dict_get_content_block_from_file_core: after add-data: e: %d\n", error_code);
...
pttui_resource_dict_integrate_with_modified_pttui_buffer_info(PttUIBuffer *head, PttUIBuffer *tail, PttUIResourceDict *resource_dict, FileInfo *file_info)
{
    ...
    for (PttUIBuffer *current_buffer = head; current_buffer && current_buffer != tail->next; current_buffer = current_buffer->next) {
        ...
        if(current_buffer->is_to_delete && current_buffer->is_new) {
            ...
            dict_buf_offset = dict_buf_next_offset;
            line_offset_dict_buf++;
            fprintf(stderr, "pttui_resource_dict.pttui_resource_dict_integrate_with_modified_pttui_buffer_info: after is-to-delete: len: %d tmp_buf: %s dict_buf_offset: %d\n", len_tmp_buf, tmp_buf, dict_buf_offset);
        }
            ...
            fprintf(stderr, "pttui_resource_dict.pttui_resource_dict_integrate_with_modified_pttui_buffer_info: after modified: len: %d tmp_buf: %s dict_buf_offset: %d\n", len_tmp_buf, tmp_buf, dict_buf_offset);
        }
