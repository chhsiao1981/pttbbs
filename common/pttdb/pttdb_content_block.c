...
Err
construct_contents_from_content_block_infos(UUID main_id, char *updater, char *update_ip, enum PttDBContentType content_type, UUID ref_id, UUID orig_content_id, enum MongoDBId mongo_db_id, int n_content_block_info, ContentBlockInfo *content_block_infos, time64_t create_milli_timestamp, int *n_line, int *n_block, int *len)
{
    Err error_code = S_OK;
    ...
    error_code = gen_content_uuid_with_db(MONGO_MAIN_CONTENT, new_content_id, create_milli_timestamp);
    if (error_code) return error_code;
...
_construct_contents_from_content_block_infos_file_core(UUID main_id, enum PttDBContentType content_type, UUID ref_id, UUID orig_content_id, int orig_block_id, int file_id, UUID new_content_id, enum MongoDBId mongo_db_id, int *n_line, int *n_block, int *len, char *line, int line_size, int *bytes_in_line, ContentBlock *content_block)
{
    ...
    char *disp_uuid2 = display_uuid(orig_content_id);
    fprintf(stderr, "pttdb_content_block._construct_contents_from_content_block_infos_file_core: after get data: main_id: %s orig_content_id: %s content_type: %d orig_block_id: %d file_id: %d n_tmp_buf: %d tmp_buf: %s e: %d\n", disp_uuid, disp_uuid2, content_type, orig_block_id, file_id, n_tmp_buf, tmp_buf, error_code);
    free(disp_uuid2);
    free(disp_uuid);

    disp_uuid = display_uuid(new_content_id);
    fprintf(stderr, "pttdb_content_block._construct_contents_from_content_block_infos_file_core: to split content core: new_content_id: %s\n", disp_uuid);
    free(disp_uuid);
    if(!error_code) {
    ...
    fprintf(stderr, "pttdb_content_block._split_contents_core: after split content core: ref_id: %s new_content_id: %s mongo_db_id: %d n_line: %d n_block: %d line: %s bytes_in_line: %d content_block: %d\n", disp_uuid, disp_uuid2, mongo_db_id, *n_line, *n_block, line, *bytes_in_line, content_block->len_block);
    free(disp_uuid2);
    free(disp_uuid);
