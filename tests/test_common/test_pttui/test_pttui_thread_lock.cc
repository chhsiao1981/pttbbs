...
{
    Err error = PttUIThreadLockWrlock(LOCK_PTTUI_EXPECTED_STATE);
    EXPECT_EQ(S_OK, error);

    pthread_rwlock_t *p_lock = NULL;
    error = PttUIThreadLockGetLock(LOCK_PTTUI_EXPECTED_STATE, &p_lock);
    EXPECT_EQ(S_OK, error);
    ...
    error = PttUIThreadLockUnlock(LOCK_PTTUI_EXPECTED_STATE);
    EXPECT_EQ(S_OK, error);
...
TEST(pttui_thread_lock, PttUIThreadLockWrlock2) {
    ...
    error = PttUIThreadLockWrlock(LOCK_PTTUI_EXPECTED_STATE);
...
{
    Err error = PttUIThreadLockRdlock(LOCK_PTTUI_EXPECTED_STATE);
    EXPECT_EQ(S_OK, error);
    ...
    error = PttUIThreadLockRdlock(LOCK_PTTUI_EXPECTED_STATE);
    EXPECT_EQ(S_OK, error);
