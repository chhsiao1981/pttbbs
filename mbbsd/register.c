...
//     < 0: error code
static int
register_email_verification(email_input_t *ein, int is_skip_check_same_email)
{
    ...
    // Nothing changed.
    if (!is_skip_check_same_email && !strcmp(email, orig)) {
	vmsg("E-Mail 與原本相同。");
	return REGISTER_ERR_CANCEL;
    }
...
check_register(void)
{
    ...
    } else if (cuser.lastlogin < FORCE_UPDATE_CONTACT_EMAIL_LASTLOGIN) {
...
//   int: 0: ok -1: err
int
change_contact_email(int is_skip_check_same_email)
{
    char email[EMAILSZ] = {};
    ...
    ein.email = email;
    ein.allow_untrusted = true;
    if (register_email_verification(&ein, is_skip_check_same_email) != REGISTER_OK)
	return -1;
